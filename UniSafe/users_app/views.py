...
class StudentProfileUpdateView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        serializer = StudentProfileUpdateSerializer(instance=request.user.student_profile, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            user = request.user
            user_serializer = StudentProfileSerializer(user)
            response_data = {"message": "Student profile details updated successfully.", "user": user_serializer.data}
...
class GenderDeskProfileUpdateView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        serializer = GenderDeskProfileUpdateSerializer(instance=request.user.genderdesk_profile, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            user = request.user
            user_serializer = GenderDeskProfileSerializer(user)
            response_data = {"message": "GenderDesk profile details updated successfully.", "user": user_serializer.data}
            return Response(response_data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ConsultantProfileUpdateView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        serializer = ConsultantProfileUpdateSerializer(instance=request.user.consultant_profile, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            user = request.user
            user_serializer = ConsultantProfileSerializer(user)
            response_data = {"message": "Consultant profile details updated successfully.", "user": user_serializer.data}
            return Response(response_data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class PoliceProfileUpdateView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        serializer = PoliceProfileUpdateSerializer(instance=request.user.police_profile, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            user = request.user
            user_serializer = PoliceProfileSerializer(user)
            response_data = {"message": "Police profile details updated successfully.", "user": user_serializer.data}
            return Response(response_data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
